...
import {
	PatchPatientHeaders,
	PatchPatientParams,
	PatchPatientReply200,
	PatchPatientReply,
	PatchPatientBody,
} from '../../schema/accounts/SchemaPatchPatient.js';
...
/**
 * @summary Route to update Patient account information. Takes an ID for Params, Body includes info to update
...
const routePatchPatient = async (
	req: FastifyRequest<{
		Headers: PatchPatientHeaders;
		Params: PatchPatientParams;
	...
	if (updater.role !== 'Patient')
	{
		rep.status(400).send({
			error: 'BAD_REQUEST',
			message: `An error occurred, you can only change patient data`,
		} as PatchPatientReply);
		return;
	}

	//Merge req.body information in the patient object
	const mergeOutcome: boolean = updater.mergeIntoPatientData(body)
	...
	} as PatchPatientReply200);
};

export default routePatchPatient;
